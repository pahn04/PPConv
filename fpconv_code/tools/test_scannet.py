...
def test(model, dst_loader, pn_list, scene_list):
    ...
    timings = []
    dummy = 0
    total_bs, total_iter = 0, 0

    scene_num = len(scene_list)
    for scene_index in range(scene_num):
        print(' ======= {}/{} ======= '.format(scene_index, scene_num), end=' ')
        ...
        if args.mode != 'test':
            ...
            print(f'accuracy: {total_correct/total_seen*100:.4f}, average batch size: {total_bs/total_iter:.2f}')
            timings_np = np.array(timings)
            print(f'Inference time : {np.mean(timings_np):.2f} +- {np.std(timings_np):.2f} / Average batch size : {total_bs/total_iter:.2f}')
