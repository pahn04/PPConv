...
parser.add_argument("--augment_type", type=str, default='rsjc')
...
def train(model, train_loader, eval_loader, tb_log, resume_epoch=0):
    ...
    for epoch in range(args.epochs):
        if epoch < resume_epoch:
            ...
            resume_epoch = 0
            continue
        log_str(f'====== Current time : {datetime.now().strftime("%m/%d %H:%M:%S")} ======')
        ...
        if (epoch >= args.start_eval_epoch and epoch % args.eval_freq == 0) or \
           (epoch > args.epochs//2 and epoch % 5 == 0):
...
if __name__ == '__main__':
    ...
    if args.augment:
        transforms = []
        if 'r' in args.augment_type: transforms.append(transform.RandomRotate(along_z=True))
        if 's' in args.augment_type: transforms.append(transform.RandomScale(scale_low=0.9, scale_high=1.1))
        if 'j' in args.augment_type: transforms.append(transform.RandomJitter(sigma=0.01, clip=0.05))
        if 'c' in args.augment_type: transforms.append(transform.RandomDropColor(color_augment=0.0))
        train_transform = transform.Compose(transforms)
    else:
