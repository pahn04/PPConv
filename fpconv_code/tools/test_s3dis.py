...
def test(model, dst_loader, pn_list, scene_list):
    ...
    for scene_index in range(scene_num):
        print(f'====== Current time : {datetime.now().strftime("%m/%d %H:%M:%S")} ======')
        ...
        print('Batch eval accuracy: %f' % (total_correct / float(total_seen)))
        print('eval point avg class IoU: %f' % (np.mean(np.array(total_correct_class) / (np.array(total_iou_deno_class, dtype=np.float) + 1e-6))))
        timings_np = np.array(timings)
        print(f'Inference time : {np.mean(timings_np):.2f} +- {np.std(timings_np):.2f} / Average batch size : {bs/it:.2f} over {it_per_scene} iterations')
    ...
    timings_np = np.array(timings)
    print(f'Inference time : {np.mean(timings_np):.2f} +- {np.std(timings_np):.2f}')
    print(f'Average batch size : {bs/it:.2f}')
