...
__all__ = ['PointNetAModule', 'PointNetSAModule', 'PointNetSAModule_RS', 'PBPSAModule', 'PointNetFPModule']
...
class PointNetSAModule_RS(nn.Module):
    def __init__(self, num_centers, radius, num_neighbors, in_channels, out_channels, include_coordinates=True):
        super().__init__()
        if not isinstance(radius, (list, tuple)):
            radius = [radius]
    ...
    def forward(self, inputs):
        features, coords = inputs
        #centers_coords = F.furthest_point_sample(coords, self.num_centers)
        weights = torch.ones((coords.shape[-1]))
        idxs = torch.multinomial(weights, self.num_centers)
        centers_coords = coords[:,:,idxs]
...
class PBPSAModule(nn.Module):
    def __init__(self, num_centers, radius, num_neighbors, in_channels, out_channels, resolution,
        ...
        for _radius, _out_channels, _num_neighbors in zip(radius, out_channels, num_neighbors):
            ...
            mlps.append(
                SharedMLP2(in_channels=in_channels + (3 if include_coordinates else 0),
                           out_channels=_out_channels, dim=2)
            )
            total_out_channels += _out_channels[-1]
        ...
        self.PBPConv_ds = Conv_pillar_ds(in_channels, total_out_channels, 3, resolution, with_se=True)
    ...
    def forward(self, inputs):
        ...
        conv_feature = self.PBPConv_ds((features, coords, centers_coords))
        """
        if len(features_list) > 1:
            return torch.cat(features_list, dim=1), centers_coords
        else:
            return features_list[0], centers_coords
        """
        return self.lrelu(features_list[0] + conv_feature), centers_coords
