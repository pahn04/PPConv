...
def prepare():
    ...
    if configs.evaluate is None:
        ...
        configs.train.log_path = os.path.join(save_path, 'log.txt')
...
def main():
    ...
    log_f = open(configs.train.log_path, 'a')

    log_string(log_f,configs)
    ...
    if os.path.exists(configs.train.checkpoint_path):
        log_string(log_f,f'==> loading checkpoint "{configs.train.checkpoint_path}"')
        checkpoint = torch.load(configs.train.checkpoint_path)
        log_string(log_f,' => loading model')
        model.load_state_dict(checkpoint.pop('model'))
        if 'optimizer' in checkpoint and checkpoint['optimizer'] is not None:
            log_string(log_f,' => loading optimizer')
        ...
        if configs.train.deep_mutual_learning:
            if 'model_student' in checkpoint and checkpoint['model_student'] is not None:
                log_string(log_f,' => loading model_student')
            ...
            if 'optimizer_student' in checkpoint and checkpoint['optimizer_student'] is not None:
                log_string(log_f,' => loading optimizer_student')
    ...
    if 'scheduler' in configs.train and configs.train.scheduler is not None:
        ...
        if 'T_max' in configs.train.scheduler:
            ...
            log_string(log_f,f'==> modify scheduler T_max to {configs.train.scheduler.T_max} {configs.train.scheduler_unit}s')
        log_string(log_f,f'==> creating scheduler "{configs.train.scheduler}" from {last_unit} {configs.train.scheduler_unit}')
    ...
    if last_epoch >= configs.train.num_epochs:
        ...
        for k, meter in meters.items():
            log_string(log_f,f'[{k}] = {meter:2f}')
        for k, meter in meters_student.items():
            log_string(log_f,f'[{k}_student] = {meter:2f}')
        return
    ...
    with tensorboardX.SummaryWriter(configs.train.save_path) as writer:
        for current_epoch in range(last_epoch + 1, configs.train.num_epochs):
            ...
            log_string(log_f, f'{datetime.now().strftime("%m-%d %H:%M:%S")}')
            ...
            log_string(log_f, f'[save_path] = {configs.train.save_path}')
    log_f.close()
