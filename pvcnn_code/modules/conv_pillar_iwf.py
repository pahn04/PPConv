...
class Conv_pillar_iwf(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, resolution, with_se=False, normalize=True, eps=1e-4, proj_axes=[1,2,3]):
        ...
        self.att_mlp1 = SharedMLP(mid_channels, 4, last_relu=False)
        self.att_mlp2 = SharedMLP(mid_channels, 4, last_relu=False)
        self.att_mlp3 = SharedMLP(mid_channels, 4, last_relu=False)
        self.att_mlp4 = SharedMLP(mid_channels, 4, last_relu=False)
    ...
    def forward(self, inputs):
        ...
        # Importance-Weighted Fusion module
        att_w = self.sigmoid(self.att_mlp1(point_feat)) + self.sigmoid(self.att_mlp2(backproj_feat[0])) + self.sigmoid(self.att_mlp3(backproj_feat[1])) + self.sigmoid(self.att_mlp4(backproj_feat[2]))
